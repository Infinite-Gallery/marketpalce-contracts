import { Marketplace, TransferProxy, ERC1155NFT } from "../typechain-types";
...
describe("erc1155 dutchauction test", function () {
    ...
    it("create test nft", async function () {
        await nft.awardItem(account1.address, 10);
        await nft.connect(account1).setApprovalForAll(transferProxy.address, true);
    });

    it("exchange", async function () {
        const sellAsset = HashAsset(0, AssetType.ERC1155, tokenId, nft.address, 10, 0);
        ...
        expect(await nft.balanceOf(account2.address, tokenId)).to.equal(10);
